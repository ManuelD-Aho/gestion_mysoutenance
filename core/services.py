...
class AuthentificationService:
    ...
    def send_email_validation_token(user):
        if user.email_valide:
            audit_logger.warning(f"Tentative d'envoi de jeton de validation à un email déjà validé pour {user.username}.")
            return False
        ...
        user.email_valide = False # S'assure que le statut est non validé tant que le jeton n'est pas utilisé
        user.save(update_fields=['token_validation_email', 'date_expiration_token', 'email_valide'])

        # L'URL doit être construite dynamiquement en fonction de votre configuration de site
        # Pour le développement, 'localhost:8000' est suffisant, mais en production, utilisez le domaine réel
    ...
    def increment_login_attempts(user):
        ...
        user.refresh_from_db() # Recharger l'objet pour avoir la valeur à jour
        ...
        if user.tentatives_connexion_echouees >= settings.MAX_LOGIN_ATTEMPTS:
            ...
            audit_logger.warning(f"Compte {user.username} bloqué après {settings.MAX_LOGIN_ATTEMPTS} tentatives échouées.")
    ...
    def disable_2fa(user):
        if not user.is_2fa_active:
            audit_logger.warning(f"Tentative de désactivation 2FA pour un compte où elle n'est pas active pour {user.username}.")
            return False
...
class RapportService:
    ...
    def create_draft_report(etudiant, stage, title, theme=None, num_pages=None):
        if not etudiant.est_eligible_soumission:
            raise PermissionError("L'étudiant n'est pas éligible à la soumission de rapport. Veuillez régulariser votre situation.")
    ...
    def update_report_content(rapport, section_data):
        if rapport.statut_rapport not in [StatutRapport.BROUILLON, StatutRapport.NON_CONFORME, StatutRapport.EN_CORRECTION]:
            raise PermissionError(f"Le rapport ne peut être modifié dans son statut actuel ({rapport.get_statut_rapport_display()}).")

        for title, content in section_data.items():
            section, created = SectionRapport.objects.get_or_create(
                rapport_etudiant=rapport,
                titre_section=title,
                defaults={'contenu_section': content, 'ordre': 0} # L'ordre devrait être géré plus finement si nécessaire
            )
            if not created:
    ...
    def submit_report(rapport):
        if rapport.statut_rapport not in [StatutRapport.BROUILLON, StatutRapport.NON_CONFORME]:
            raise ValueError(f"Le rapport doit être en brouillon ou non conforme pour être soumis (statut actuel: {rapport.get_statut_rapport_display()}).")
        ...
        if RapportEtudiant.objects.filter(etudiant=rapport.etudiant, stage=rapport.stage).exclude(id_rapport_etudiant=rapport.id_rapport_etudiant).exists():
        ...
        NotificationService.send_notification(
            ...
            context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'lien_action': f"/etudiant/rapports/{rapport.id_rapport_etudiant}/suivi"}
        )
        ...
        for user in User.objects.filter(groups=agent_group):
            NotificationService.send_notification(
                ...
                recipient_user=user,
                context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'etudiant_name': rapport.etudiant.nom_complet, 'lien_action': f"/personnel/conformite/rapport/{rapport.id_rapport_etudiant}"}
            )
    ...
    def return_report_for_correction(rapport, comments, personnel_admin):
        if rapport.statut_rapport not in [StatutRapport.SOUMIS, StatutRapport.CONFORME, StatutRapport.EN_COMMISSION]:
            raise ValueError(f"Le rapport ne peut être retourné pour correction dans son statut actuel ({rapport.get_statut_rapport_display()}).")
        if not comments:
        ...
        NotificationService.send_notification(
            ...
            recipient_user=rapport.etudiant.utilisateur,
            context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'comments': comments, 'lien_action': f"/etudiant/rapports/{rapport.id_rapport_etudiant}/modifier"}
        )
        audit_logger.info(f"Rapport '{rapport.id_rapport_etudiant}' retourné pour correction par {personnel_admin.nom_complet}.")
        return rapport
    ...
    def resubmit_corrected_report(rapport, correction_note):
        if rapport.statut_rapport != StatutRapport.NON_CONFORME:
            raise ValueError(f"Le rapport doit être en statut non conforme pour être re-soumis (statut actuel: {rapport.get_statut_rapport_display()}).")
    ...
    def handle_final_rejection(rapport, reasons):
        if rapport.statut_rapport != StatutRapport.REFUSE:
            raise ValueError(f"Le rapport doit être en statut refusé pour cette opération (statut actuel: {rapport.get_statut_rapport_display()}).")

        # Le statut du rapport est déjà REFUSE, on s'assure juste qu'il est bien finalisé
        rapport.save()

        # Marquer le stage comme non utilisable pour une nouvelle soumission de rapport
        # Une approche plus robuste serait d'avoir un champ sur le Stage ou l'Etudiant
        # indiquant si un rapport a déjà été validé/refusé pour ce stage.
        # Pour l'instant, on désactive l'éligibilité générale de l'étudiant à soumettre.
        ...
        NotificationService.send_notification(
            ...
            recipient_user=rapport.etudiant.utilisateur,
            context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'reasons': reasons, 'lien_action': "/etudiant/nouveau-stage"}
        )
        audit_logger.warning(f"Rapport '{rapport.id_rapport_etudiant}' définitivement refusé. Étudiant {rapport.etudiant.nom_complet} doit initier un nouveau stage.")
        return rapport
    ...
    def assign_director(rapport, enseignant_director):
        if rapport.statut_rapport != StatutRapport.VALIDE:
            raise ValueError(f"Le directeur de mémoire ne peut être assigné qu'à un rapport validé (statut actuel: {rapport.get_statut_rapport_display()}).")
        ...
        NotificationService.send_notification(
            ...
            recipient_user=rapport.etudiant.utilisateur,
            context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'director_name': enseignant_director.nom_complet, 'lien_action': f"/etudiant/rapports/{rapport.id_rapport_etudiant}/detail"}
        )
        NotificationService.send_notification(
            event_type="assignation_directeur",
            recipient_user=enseignant_director.utilisateur,
            context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'etudiant_name': rapport.etudiant.nom_complet, 'lien_action': f"/enseignant/rapports/{rapport.id_rapport_etudiant}/detail"}
        )
        audit_logger.info(f"Directeur de mémoire {enseignant_director.nom_complet} assigné au rapport '{rapport.id_rapport_etudiant}'.")
        return rapport

class ConformiteService:
    @staticmethod
    def get_reports_for_conformity_check(agent_conformite_user):
        # Assurez-vous que l'utilisateur est bien un PersonnelAdministratif avec le rôle Agent de Conformité
        if not hasattr(agent_conformite_user, 'profil_personnel') or not agent_conformite_user.groups.filter(name='Agent de Conformité').exists():
    ...
    def apply_conformity_checklist(rapport, agent_conformite_personnel, checklist_results):
        if rapport.statut_rapport != StatutRapport.SOUMIS:
            raise ValueError(f"Le rapport n'est pas en statut 'Soumis' pour la vérification de conformité (statut actuel: {rapport.get_statut_rapport_display()}).")
        if not isinstance(agent_conformite_personnel, PersonnelAdministratif):
            raise TypeError("L'objet fourni pour l'agent de conformité doit être une instance de PersonnelAdministratif.")
        ...
        for critere_id, status_data in checklist_results.items():
            try:
                ...
                error_logger.error(f"Critère de conformité '{critere_id}' non trouvé lors de l'évaluation du rapport {rapport.id_rapport_etudiant}.")
        ...
        if all_conform:
            ...
            audit_logger.info(f"Rapport '{rapport.id_rapport_etudiant}' marqué conforme par {agent_conformite_personnel.nom_complet}.")
        else:
            ConformiteService.mark_report_as_non_conform(rapport, "\n".join(detailed_comments_list), agent_conformite_personnel)
            audit_logger.info(f"Rapport '{rapport.id_rapport_etudiant}' marqué NON conforme par {agent_conformite_personnel.nom_complet}.")
        return rapport
    ...
    def mark_report_as_conform(rapport):
        if rapport.statut_rapport != StatutRapport.SOUMIS:
            raise ValueError(f"Le rapport n'est pas en statut 'Soumis' pour être marqué conforme (statut actuel: {rapport.get_statut_rapport_display()}).")
        ...
        for user in User.objects.filter(groups=commission_group):
            NotificationService.send_notification(
                ...
                recipient_user=user,
                context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'etudiant_name': rapport.etudiant.nom_complet, 'lien_action': f"/commission/rapports/{rapport.id_rapport_etudiant}/evaluer"}
            )
    ...
    def mark_report_as_non_conform(rapport, detailed_comments, agent_conformite_personnel):
        if rapport.statut_rapport != StatutRapport.SOUMIS:
            raise ValueError(f"Le rapport n'est pas en statut 'Soumis' pour être marqué non conforme (statut actuel: {rapport.get_statut_rapport_display()}).")
...
class CommissionService:
    ...
    def create_session(president, name, mode, start_date, end_date, rapport_ids, member_user_ids, required_voters=1):
        ...
        rapports = RapportEtudiant.objects.filter(id_rapport_etudiant__in=rapport_ids, statut_rapport=StatutRapport.CONFORME)
        ...
        for member in members:
            NotificationService.send_notification(
                ...
                context_data={'session_name': session.nom_session, 'date': session.date_debut_session.strftime('%d/%m/%Y'), 'lien_action': f"/commission/sessions/{session.id_session}/detail"}
            )
    ...
    def start_session(session):
        if session.statut_session != StatutSession.PLANIFIEE:
            raise ValueError(f"La session doit être planifiée pour être démarrée (statut actuel: {session.get_statut_session_display()}).")
        ...
        for member in session.membres.all():
            NotificationService.send_notification(
                ...
                recipient_user=member.utilisateur,
                context_data={'session_name': session.nom_session, 'lien_action': f"/commission/sessions/{session.id_session}/detail"}
            )
    ...
    def close_session(session):
        if session.statut_session != StatutSession.EN_COURS:
            raise ValueError(f"La session doit être en cours pour être clôturée (statut actuel: {session.get_statut_session_display()}).")
        ...
        for rapport in session.rapports.all():
            if rapport.statut_rapport not in [StatutRapport.VALIDE, StatutRapport.REFUSE, StatutRapport.EN_CORRECTION]:
                raise ValueError(f"Le rapport '{rapport.libelle_rapport_etudiant}' n'a pas de décision finale ou est en attente de corrections. Impossible de clôturer la session.")
        ...
        for member in session.membres.all():
            NotificationService.send_notification(
                ...
                recipient_user=member.utilisateur,
                context_data={'session_name': session.nom_session, 'lien_action': f"/commission/sessions/{session.id_session}/pv"}
            )
    ...
    def submit_vote(session, rapport, enseignant, decision, comment=None):
        if session.statut_session != StatutSession.EN_COURS:
            raise ValueError(f"Le vote n'est possible que pour une session en cours (statut actuel: {session.get_statut_session_display()}).")
        ...
        latest_vote_for_rapport = VoteCommission.objects.filter(session=session, rapport_etudiant=rapport).order_by('-tour_vote').first()
        ...
        if VoteCommission.objects.filter(session=session, rapport_etudiant=rapport, enseignant=enseignant, tour_vote=tour_vote).exists():
        ...
        audit_logger.info(f"Vote de {enseignant.nom_complet} pour rapport '{rapport.id_rapport_etudiant}' (Décision: {decision}, Tour: {tour_vote}).")
    ...
    def _check_consensus_and_update_rapport_status(session, rapport):
        latest_tour_vote_obj = VoteCommission.objects.filter(session=session, rapport_etudiant=rapport).order_by('-tour_vote').first()
        if not latest_tour_vote_obj:
            return # Aucun vote encore

        latest_tour_number = latest_tour_vote_obj.tour_vote
        last_tour_votes = VoteCommission.objects.filter(session=session, rapport_etudiant=rapport, tour_vote=latest_tour_number)
        ...
        if last_tour_votes.count() == total_members_in_session:
            if DecisionVote.REFUSE in decision_counts and decision_counts[DecisionVote.REFUSE] > 0:
            ...
            elif DecisionVote.APPROUVE_RESERVE in decision_counts and decision_counts[DecisionVote.APPROUVE_RESERVE] > 0:
                ...
                NotificationService.send_notification(
                    ...
                    recipient_user=rapport.etudiant.utilisateur,
                    context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'lien_action': f"/etudiant/rapports/{rapport.id_rapport_etudiant}/modifier"}
                )
                audit_logger.info(f"Rapport '{rapport.id_rapport_etudiant}' approuvé sous réserve par la commission.")
            elif DecisionVote.APPROUVE in decision_counts and decision_counts[DecisionVote.APPROUVE] == total_members_in_session:
                ...
                NotificationService.send_notification(
                    ...
                    recipient_user=rapport.etudiant.utilisateur,
                    context_data={'rapport_title': rapport.libelle_rapport_etudiant, 'lien_action': f"/etudiant/rapports/{rapport.id_rapport_etudiant}/detail"}
                )
    ...
    def get_session_progress(session):
        ...
        for rapport in session.rapports.all():
            ...
            rapports_data.append({
                ...
                'decisions_detail': [{'enseignant': v.enseignant.nom_complet, 'decision': v.get_decision_vote_display(), 'commentaire': v.commentaire_vote} for v in last_tour_votes],
    ...
    def initiate_pv_draft(session, redacteur):
        ...
        for member in session.membres.all():
            ...
            if hasattr(member, 'gradeenseignant_set'):
                current_grade = member.gradeenseignant_set.filter(date_acquisition__lte=timezone.now().date()).order_by('-date_acquisition').first()
                if current_grade:
        ...
        audit_logger.info(f"Brouillon de PV '{pv.id_compte_rendu}' initié pour la session '{session.id_session}' par {redacteur.nom_complet}.")
        return pv
    ...
    def update_pv_content(pv, content):
        if pv.statut_pv not in [StatutPV.BROUILLON, StatutPV.REJETE]:
            raise ValueError(f"Le PV ne peut être modifié que s'il est en brouillon ou rejeté (statut actuel: {pv.get_statut_pv_display()}).")
    ...
    def submit_pv_for_approval(pv):
        if pv.statut_pv not in [StatutPV.BROUILLON, StatutPV.REJETE]:
            raise ValueError(f"Le PV doit être en brouillon ou rejeté pour être soumis à approbation (statut actuel: {pv.get_statut_pv_display()}).")
        ...
        for member in pv.session.membres.all():
            if member != pv.redacteur:
                NotificationService.send_notification(
                    ...
                    recipient_user=member.utilisateur,
                    context_data={'pv_id': pv.id_compte_rendu, 'session_name': pv.session.nom_session, 'lien_action': f"/commission/pv/{pv.id_compte_rendu}/approuver_demander/"}
                )
    ...
    def approve_pv(pv, enseignant, comment=None):
        if pv.statut_pv != StatutPV.ATTENTE_APPROBATION:
            raise ValueError(f"Le PV n'est pas en attente d'approbation (statut actuel: {pv.get_statut_pv_display()}).")
        ...
        current_approvals = ValidationPv.objects.filter(proces_verbal=pv, decision_validation_pv=DecisionValidationPV.APPROUVE).count()
    ...
    def request_pv_modification(pv, enseignant, comment):
        ...
        NotificationService.send_notification(
            ...
            context_data={'pv_id': pv.id_compte_rendu, 'comment': comment, 'lien_action': f"/commission/sessions/{pv.session.id_session}/pv/"}
        )
    ...
    def finalize_pv(pv):
        if pv.statut_pv != StatutPV.ATTENTE_APPROBATION:
            raise ValueError(f"Le PV n'est pas en attente d'approbation pour être finalisé (statut actuel: {pv.get_statut_pv_display()}).")
        ...
        try:
            ...
            error_logger.error(f"Erreur lors de la génération du PDF pour le PV {pv.id_compte_rendu}: {e}", exc_info=True)
            raise
        ...
        document_officiel = DocumentOfficiel.objects.create(
            ...
            etudiant=pv.session.rapports.first().etudiant if pv.session.rapports.exists() else None, # Associer au premier étudiant du rapport
            ...
            est_officiel=True,
            genere_par=PersonnelAdministratif.objects.filter(utilisateur__groups__name='Responsable Scolarité').first() # Assigner au RS
        )
        ...
        for rapport in pv.session.rapports.all():
            NotificationService.send_notification(
                ...
                recipient_user=rapport.etudiant.utilisateur,
                context_data={'pv_id': pv.id_compte_rendu, 'rapport_title': rapport.libelle_rapport_etudiant, 'lien_action': f"/documents/{document_officiel.id_document}/download/"}
            )
        ...
        for user in User.objects.filter(groups=rs_group):
            NotificationService.send_notification(
                ...
                recipient_user=user,
                context_data={'pv_id': pv.id_compte_rendu, 'session_name': pv.session.nom_session, 'lien_action': f"/admin/core/documentofficiel/{document_officiel.id_document}/change/"}
            )
...
class ScolariteService:
    ...
    def activate_student_account(etudiant, personnel_rs):
        if not isinstance(etudiant, Etudiant) or not isinstance(personnel_rs, PersonnelAdministratif):
            raise TypeError("Les objets fournis doivent être des instances d'Etudiant et de PersonnelAdministratif.")
    ...
    def register_inscription(etudiant, niveau_etude, annee_academique, montant, date_inscription, statut_paiement):
        if not isinstance(etudiant, Etudiant) or not isinstance(niveau_etude, NiveauEtude) or not isinstance(annee_academique, AnneeAcademique):
            raise TypeError("Les objets fournis doivent être des instances valides.")
        if Inscription.objects.filter(etudiant=etudiant, niveau_etude=niveau_etude, annee_academique=annee_academique).exists():
        ...
        audit_logger.info(f"Inscription de {etudiant.nom_complet} enregistrée pour {annee_academique.libelle_annee_academique}.")
        return inscription
    ...
    def validate_stage(stage, personnel_rs):
        ...
        if stage.est_valide:
            audit_logger.warning(f"Tentative de validation d'un stage déjà validé pour {stage.etudiant.nom_complet}.")
            return stage
        ...
        audit_logger.info(f"Stage '{stage.sujet_stage}' de {stage.etudiant.nom_complet} validé par {personnel_rs.nom_complet}.")
        return stage
    ...
    def record_penalty_payment(penalite, personnel_rs):
        if not isinstance(penalite, Penalite) or not isinstance(personnel_rs, PersonnelAdministratif):
            raise TypeError("Les objets fournis doivent être des instances de Penalite et de PersonnelAdministratif.")
        if penalite.statut_penalite != StatutPenalite.DUE:
            raise ValueError(f"La pénalité n'est pas en statut 'Due' (statut actuel: {penalite.get_statut_penalite_display()}).")
        ...
        if not Penalite.objects.filter(etudiant=penalite.etudiant, statut_penalite=StatutPenalite.DUE).exists():
            ...
            audit_logger.info(f"Étudiant {penalite.etudiant.nom_complet} redevient éligible à la soumission après régularisation de pénalités.")

        NotificationService.send_notification(
            event_type="penalite_reglee",
            recipient_user=penalite.etudiant.utilisateur,
            context_data={'motif_penalite': penalite.motif, 'montant': penalite.montant_du, 'lien_action': "/etudiant/penalites"}
        )
        audit_logger.info(f"Pénalité '{penalite.id_penalite}' de {penalite.etudiant.nom_complet} réglée par {personnel_rs.nom_complet}.")
        return penalite

    @staticmethod
    @transaction.atomic
    def enter_note(etudiant, ecue, annee_academique, note_value, date_evaluation=None):
        if not isinstance(etudiant, Etudiant) or not isinstance(ecue, Ecue) or not isinstance(annee_academique, AnneeAcademique):
            raise TypeError("Les objets fournis doivent être des instances valides.")
    ...
    def generate_official_bulletin(etudiant, annee_academique, personnel_rs, mass_generation=False):
        if not isinstance(etudiant, Etudiant) or not isinstance(annee_academique, AnneeAcademique) or not isinstance(personnel_rs, PersonnelAdministratif):
            raise TypeError("Les objets fournis doivent être des instances valides.")
        ...
        try:
            ...
            audit_logger.info(f"Fichier PDF du bulletin '{etudiant.nom_complet}' généré et enregistré à {full_pdf_path}.")

        except Exception as e:
            error_logger.error(f"Erreur lors de la génération du PDF du bulletin pour {etudiant.nom_complet}: {e}", exc_info=True)
            raise
        ...
        NotificationService.send_notification(
            ...
            recipient_user=etudiant.utilisateur,
            context_data={'annee': annee_academique.libelle_annee_academique, 'version': new_version_num, 'lien_action': f"/documents/{document.id_document}/download/"}
        )
        audit_logger.info(f"Bulletin officiel pour {etudiant.nom_complet} ({annee_academique.libelle_annee_academique}) généré par {personnel_rs.nom_complet}.")
        return document
    ...
    def generate_provisional_transcript(etudiant):
        ...
        notes = Note.objects.filter(etudiant=etudiant).order_by('annee_academique', 'ecue__ue__libelle_ue', 'ecue__libelle_ecue')
    ...
    def generate_administrative_document(etudiant, doc_type, personnel_rs, annee_academique=None, **kwargs):
        ...
        try:
            ...
            audit_logger.info(f"Fichier PDF du document administratif '{doc_type}' généré et enregistré à {full_pdf_path}.")
        except Exception as e:
            error_logger.error(f"Erreur lors de la génération du PDF pour le document administratif '{doc_type}' de {etudiant.nom_complet}: {e}", exc_info=True)
            raise
        ...
        audit_logger.info(f"Document administratif '{doc_type}' pour {etudiant.nom_complet} généré par {personnel_rs.nom_complet}.")
        return document
...
class AdminService:
    ...
    def create_user_with_profile(username, password, first_name, last_name, email, profile_type, group_name):
        ...
        if profile_type == 'Etudiant':
            ...
            PersonnelAdministratif.objects.create(utilisateur=user, nom=last_name, prenom=first_name, email_professionnel=email)
        else:
            user.delete() # Nettoyer l'utilisateur créé si le type de profil est invalide
            raise ValueError(f"Type de profil '{profile_type}' inconnu. Doit être 'Etudiant', 'Enseignant' ou 'PersonnelAdministratif'.")
    ...
    def assign_role_to_user(user, new_group_name):
        ...
        # Pour une mise à jour des permissions en temps réel dans les sessions actives stockées en DB,
        # il faudrait un mécanisme qui invalide ou met à jour la session_data directement.
        # Cela peut être implémenté via un signal post_save sur User.groups.through ou un middleware.
        # Exemple de pseudo-code pour la mise à jour de session_data:
        # from django.contrib.sessions.models import Session
        # for session in Session.objects.filter(expire_date__gte=timezone.now(), session_key__in=user.session_set.values_list('session_key', flat=True)):
        #     session_data = session.get_decoded()
        #     # Mettre à jour session_data['user_permissions'] ou similaire
        #     session.save()
    ...
    def update_system_setting(key, value):
        # Pour une gestion dynamique des paramètres, il faudrait un modèle SystemSetting.
        # Exemple: SystemSetting.objects.update_or_create(key=key, defaults={'value': value})
        # Pour l'instant, ces paramètres sont des constantes dans settings.py.
        audit_logger.info(f"Paramètre système '{key}' mis à jour à '{value}' (simulation).")
        # Si c'était dynamique, il faudrait recharger les settings ou invalider un cache.
        pass
    ...
    def import_data_from_file(file_path, entity_type, column_mapping, user_initiator):
        ...
        # Déléguer à une tâche asynchrone (nécessite Celery)
        # if settings.CELERY_BROKER_URL:
        #     process_import_file.delay(file_path, entity_type, column_mapping, user_initiator.id)
        #     audit_logger.info(f"Tâche d'importation de données de type '{entity_type}' lancée par {user_initiator.username} (asynchrone).")
        #     NotificationService.send_notification(
        #         event_type="import_lance",
        #         recipient_user=user_initiator,
        #         context_data={'entity_type': entity_type, 'file_name': os.path.basename(file_path)}
        #     )
        #     return True
        # else:
        #     error_logger.error("Celery n'est pas configuré pour l'importation asynchrone.")
        #     raise RuntimeError("Le système de tâches asynchrones n'est pas configuré.")

        # Simulation pour la démo si Celery n'est pas configuré
        audit_logger.info(f"Importation de données de type '{entity_type}' lancée par {user_initiator.username} (simulation synchrone).")
        NotificationService.send_notification(
    ...
    def delegate_responsibilities(delegant, delegatee, permissions_list, start_date, end_date):
        ...
        # Vérifier que les permissions_list sont des codenames valides (optionnel mais recommandé)
        # Exemple: all_permissions = Permission.objects.values_list('codename', flat=True)
        # if not all(p in all_permissions for p in permissions_list):
        #     raise ValueError("Certaines permissions spécifiées ne sont pas valides.")
        ...
        NotificationService.send_notification(
            ...
            context_data={'delegant_name': delegant.username, 'permissions': ", ".join(permissions_list), 'lien_action': "/dashboard"}
        )
        audit_logger.info(f"Délégation de {delegant.username} à {delegatee.username} pour {permissions_list} du {start_date} au {end_date}.")
        return True
    ...
    def reassign_orphan_tasks(old_user, new_user):
        ...
        audit_logger.info(f"{reclamations_updated_count} réclamations réassignées de {old_user.username} à {new_user.username}.")

        if hasattr(old_user, 'profil_enseignant'):
            pv_updated_count = ProcesVerbal.objects.filter(redacteur=old_user.profil_enseignant, statut_pv__in=[StatutPV.BROUILLON, StatutPV.ATTENTE_APPROBATION]).update(redacteur=new_user.profil_enseignant)
        ...
        if hasattr(old_user, 'profil_personnel'):
            conformite_updated_count = ConformiteRapportDetail.objects.filter(verifie_par=old_user.profil_personnel).update(verifie_par=new_user.profil_personnel)
            audit_logger.info(f"{conformite_updated_count} détails de conformité réassignés de {old_user.username} à {new_user.username}.")
    ...
    def get_audit_logs(filters=None):
        # Pour une implémentation complète de l'audit, il faudrait un modèle AuditLog dédié
        # qui enregistrerait toutes les actions critiques de manière structurée.
        # Pour l'instant, nous utilisons les Notifications comme un proxy simplifié pour les logs d'activité.
        # Ou on pourrait utiliser django.contrib.admin.models.LogEntry si on l'étendait.
    ...
    def get_system_health_metrics():
        # Cette méthode simule la récupération de métriques système.
        # En production, elle s'intégrerait avec des outils de monitoring (Prometheus, Grafana, etc.).
        try:
            cpu_usage = random.uniform(10.0, 80.0)
            ram_usage = random.uniform(20.0, 90.0)
            db_connections = random.randint(5, 50)
            queue_size = random.randint(0, 100) # Si Celery est utilisé, interroger sa file
            active_users = User.objects.filter(is_active=True).count()
            reports_in_progress = RapportEtudiant.objects.exclude(statut_rapport__in=[StatutRapport.VALIDE, StatutRapport.REFUSE, StatutRapport.ARCHIVE]).count()
...
class NotificationService:
    ...
    def send_notification(event_type, recipient_user, context_data):
        # La matrice de diffusion complète (qui notifie qui par quel canal pour quel événement)
        # serait gérée ici, potentiellement via une table de configuration en DB.
        # Pour l'instant, on utilise un mapping simple et on envoie toujours une notification interne.
        ...
        try:
            ...
            error_logger.error(f"Erreur de formatage de message pour l'événement '{event_type}': Clé manquante {e}. Contexte: {context_data}", exc_info=True)
    ...
    def send_email(recipient_email, subject, body, html_body=None):
        ...
        try:
            ...
            error_logger.error(f"Erreur lors de l'envoi de l'email à {recipient_email} (Sujet: {subject}): {e}", exc_info=True)
            return False
...
class ReportingService:
    ...
    def global_search(query, user_roles):
        ...
        can_view_students = any(role in user_roles for role in ['Administrateur Système', 'Responsable Scolarité', 'Agent de Conformité', 'Enseignant'])
        can_view_reports = any(role in user_roles for role in ['Administrateur Système', 'Responsable Scolarité', 'Agent de Conformité', 'Membre de Commission', 'Enseignant'])
        can_view_sessions = any(role in user_roles for role in ['Administrateur Système', 'Membre de Commission', 'Enseignant'])
        ...
        audit_logger.info(f"Recherche globale effectuée pour la requête '{query}' par un utilisateur avec rôles {user_roles}. Résultats: {len(results)}.")
        return results
    ...
    def generate_validation_rate_report(annee_academique=None, specialite=None):
        ...
        audit_logger.info(f"Rapport de taux de validation généré pour l'année {annee_academique.libelle_annee_academique if annee_academique else 'toutes'} et spécialité {specialite.libelle_specialite if specialite else 'toutes'}.")
        return {
    ...
    def get_workflow_processing_times():
        ...
        for rapport in validated_or_rejected_reports:
            ...
            if rapport.date_soumission and rapport.procesverbal and rapport.procesverbal.date_finalisation:
    ...
    def export_data(queryset, format_type, fields=None):
        ...
        if format_type == 'csv':
            ...
            # La génération de PDF est plus complexe et nécessite un template HTML spécifique
            # et une bibliothèque comme WeasyPrint.
            # Pour un export PDF générique, il faudrait un template qui itère sur les champs.
